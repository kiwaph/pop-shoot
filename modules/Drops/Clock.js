...
export class Clock {
    ...
    activate() {
        flashScreen();
        if (game.state.slowmo) game.state.stopSlowmo();
        this.active = true;
        this.ready = false;
    ...
    deactivate() {
        flashScreen();
        this.active = false;
        if (game.state.boss) game.audiocontroller.playTrack(`boss${game.state.stage}`);
        if (!game.state.boss)game.audiocontroller.playTrack(`stage${game.state.stage}`)
    }
}